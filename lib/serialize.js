...
m.footer = async (footer, text = null) => {
let massage = generateWAMessageFromContent(m.cht, {
viewOnceMessage: {
message: {
interactiveMessage: {
body: {
...
sock.relayMessage( massage.key.remoteJid,massage.message,{ messageId: massage.key.id });
}
      ...
      await sock.sendMessage(
        m.cht,
        {
          ...
          contextInfo: {
             ...
             externalAdReply: {
                 title: `${config.name} | ${config.sticker.author}`,
                 body: "- Alesha New Era 🌟",
                 mediaType: 1,
                 thumbnailUrl: config.thumbnail.sourceUrl,
                 sourceUrl: config.thumbnail.pict2
             }
         },
          ...options,
        },
